	...
	<div class="ibox float-e-margins settings-box">
		...
		<div class="ibox-content">
				...
				?>
					...
					<div class="col-lg-12 col-md-12">
						<div class="alert alert-warning">
							<i class="fa fa-warning m-r-xs"></i> <?php __('notifications_sms_na'); ?> <a href="<?php echo $_SERVER['PHP_SELF']; ?>?controller=pjBaseSms&action=pjActionIndex"><?php __('notifications_sms_na_here'); ?></a>
						</div>
					</div>
				</div>
				<?php
				...
				<div class="notification-area<?php echo $tpl['arr']['is_active'] && $is_ready ? NULL : ' hidden'; ?>">
					<div class="form-group">
						...
						foreach ($tpl['lp_arr'] as $v)
						{
							...
							<div class="<?php echo $tpl['is_flag_ready'] ? 'input-group ' : NULL;?>pj-multilang-wrap" data-index="<?php echo $v['id']; ?>" style="display: <?php echo (int) $v['is_default'] === 0 ? 'none' : NULL; ?>">
								<input type="text" name="i18n[<?php echo $v['id']; ?>][subject]" class="form-control" value="<?php echo isset($tpl['arr']['i18n'][$v['id']]['subject']) ? pjSanitize::html($tpl['arr']['i18n'][$v['id']]['subject']) : ''; ?>">
					...
					<div class="form-group">
						...
						foreach ($tpl['lp_arr'] as $v)
						{
							?>
							<div class="<?php echo $tpl['is_flag_ready'] ? 'input-group ' : NULL;?>pj-multilang-wrap" data-index="<?php echo $v['id']; ?>" style="display: <?php echo (int) $v['is_default'] === 0 ? 'none' : NULL; ?>">
								<textarea name="i18n[<?php echo $v['id']; ?>][message]" class="form-control mceEditor"><?php echo isset($tpl['arr']['i18n'][$v['id']]['message']) && !empty($tpl['arr']['i18n'][$v['id']]['message']) ? htmlspecialchars(stripslashes($tpl['arr']['i18n'][$v['id']]['message'])) : ''; ?></textarea>
								<?php if ($tpl['is_flag_ready']) : ?>
								<span class="input-group-addon pj-multilang-input"><img src="<?php echo PJ_INSTALL_URL . PJ_FRAMEWORK_LIBS_PATH . 'pj/img/flags/' . $v['file']; ?>" alt="<?php echo pjSanitize::html($v['name']); ?>"></span>
								<?php endif; ?>
							</div>
			...
			<div class="notification-area<?php echo $tpl['arr']['is_active'] && $is_ready ? NULL : ' hidden'; ?>">
				...
				<button type="submit" class="ladda-button btn btn-primary btn-lg btn-phpjabbers-loader" data-style="zoom-in" style="margin-right: 15px;">
