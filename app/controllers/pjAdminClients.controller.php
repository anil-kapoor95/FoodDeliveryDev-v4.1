...
class pjAdminClients extends pjAdmin
{
	...
	public function pjActionGetClient()
	{
		...
		if ($this->isXHR())
		{
			...
			$column = 'c_name';
			$direction = 'ASC';
			$allowed_columns = array('c_name', 'c_email', 'cnt_orders', 'status', 'locked');
			...
			$data = $pjClientModel
			->select("t1.id, t2.email AS c_email, t2.name AS c_name, t2.status, t2.locked, (SELECT COUNT(TO.client_id) FROM `".pjOrderModel::factory()->getTable()."` AS `TO` WHERE `TO`.client_id=t1.id) AS cnt_orders")
